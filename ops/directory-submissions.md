# DeltaIQ directory submissions

## Compliant short description (copy/paste)

DeltaIQ is a rules-based publisher of observational SPY/SPX market-condition information for short-dated (0DTE/1DTE) options context. Subscribers receive identical dashboard publications when defined conditions occur. Optional SMS only notifies you to sign in. Educational and informational only — not investment advice.

## Links

- Homepage: https://getdeltaiq.com/
- Guides: https://getdeltaiq.com/guides/
- About: https://getdeltaiq.com/about.html
- Contact email: support@getdeltaiq.com
- Company: DeltaIQ, LLC
- Founder: Timothy Ong
- Categories (typical): Market data / Trading tools / Financial education / Fintech SaaS
- Social: https://www.linkedin.com/company/deltaiqllc/ · https://x.com/getdeltaiq

## Shorter blurb (≤160 chars)

Rules-based SPY/SPX market-condition publisher for 0DTE/1DTE context. Educational only — not investment advice. https://getdeltaiq.com/

## Submitted (2026-09-12)

| Directory | Status | Notes |
|-----------|--------|-------|
| SaaSHub | Submitted — pending free review (up to ~32 days) | Manage: https://www.saashub.com/manage/deltaiq · Expected page after approval: https://www.saashub.com/deltaiq-alternatives |
| Startup Stash | Submitted — awaiting reply | Applied via https://startupstash.com/add-listing/ |

## Remaining (operator login / CAPTCHA usually required)

| Directory | URL | Notes |
|-----------|-----|-------|
| G2 | https://www.g2.com/products/new | Vendor claim / Digital Markets |
| Capterra | https://www.capterra.com/vendors/sign-up | Software listing |
| Product Hunt | https://www.producthunt.com/posts/new | Launch listing |
| AlternativeTo | https://alternativeto.net/account/register | Add alternative |
| SourceForge | https://sourceforge.net/software/create-project/ | Software directory |
| BetaList | https://betalist.com/submit | Startup listing |
| Indie Hackers | https://www.indiehackers.com/products | Product page |
| Curlie | https://curlie.org/docs/en/add.html | Editorial directory |
| There's An AI For That | N/A | Skip — not an AI-tool primary |

## Do not claim

- Investment advice, advisory relationship, guaranteed returns
- Trade alerts with strikes/size/timing
- Brokerage, RIA, or “signals that tell you what to buy”
